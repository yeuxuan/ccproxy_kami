...
if(isset($_POST['username']) && isset($_POST['password'])){
    ...
    $row=$DB->selectRow("SELECT * FROM sup_admin WHERE username='$user' and password='$pass' limit 1");
	...
	if ((!$code || strtolower($code) != $_SESSION['xx_session_code'])) {
		...
		setcookie("sup_admin_token", $cookies, time() + 604800);
		$DB->exe("update sup_admin set cookies='$cookies' where username='{$row['username']}'");
		@header('Content-Type: text/html; charset=UTF-8');
	...
	setcookie("sup_admin_token", "", time() - 604800);
	@header('Content-Type: text/html; charset=UTF-8');
			...
			<div class="layout-content layui-form layui-form-pane">
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<input type="text" name="username" lay-verify="required" class="layui-input" placeholder="请输入用户名" title="用户名">
					</div>
				</div>
				<div class="layui-form-item">
					<label class="layui-form-label"><i class="layui-icon layui-icon-password"></i></label>
					<div class="layui-input-block">
						<input type="password" name="password" lay-verify="required" class="layui-input" placeholder="请输入密码" title="登录密码">
					</div>
				</div>
                <div class="layui-form-item">
					<label class="layui-form-label"><i class="layui-icon layui-icon-vercode"></i></label>
					<div class="layui-input-block">
						<input style="width:50%;display:inline" type="text" name="code" lay-verify="required" class="layui-input" placeholder="请输入验证码" title="验证码">
                        <img style="float: right;" src="./code.php?r=<?php echo time();?>" width="45%" height="38"onclick="this.src='./code.php?r='+Math.random();" title="点击更换验证码">
                    </div> 
